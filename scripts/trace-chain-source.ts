/**
 * 精确追踪 chain_native 存款跳变来源
 * 在 commitYear 前后记录存款，并打印结算 result 的关键分项
 */
...
function runTrace(tag: string) {
  ...
  for (let age = 22; age <= 62; age++) {
    ...
    const yrSavingsChange = yr?.netChange ?? '?'
    const yrPassive = yr?.passiveIncome ?? '?'
    const yrChain = yr?.chainHoldingsGain ?? '?'
    const yrSide = yr?.sideHustleIncome ?? '?'

    if (savingsDelta < -1000000 || savingsDelta > 1000000 || holdingsDelta < -1000000 || holdingsDelta > 1000000) {
      console.log(`年龄${age} | 事件:${eventTitle || crossTitle || '-'}(${evId})`)
      ...
      console.log(`  结算 netChange:${yrSavingsChange} passive:${yrPassive} chainGain:${yrChain} side:${yrSide}`)
    }
  }
}
