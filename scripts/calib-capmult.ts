/**
 * 参数化校准：被动收入资本化倍数对成功率的影响
 * 复用修正后的模拟器（commitYear 后 dismissYearEnd 抽事件）
 * 遍历 passiveIncomeCapMult ∈ {20, 14, 12, 10, 8}，测各路径成功率
 * 找到能收敛到 60 左右的组合
 */
...
import { checkCanRetire, calculateTotalWealth, setPassiveIncomeCapMult } from '../src/utils/math-engine.js';
...
console.log('  参数化校准 · 被动收入资本化倍数 passiveIncomeCapMult');
console.log('  修正模拟器 · 最优策略 · 60轮/路径/倍数');
console.log('='.repeat(90));

const RESULTS: { mult: number; paths: Record<string, any> }[] = [];
for (const mult of [20, 14, 12, 10, 8]) {
  setPassiveIncomeCapMult(mult);
...
console.log('='.repeat(90));
console.log('  倍数  ' + PATHS.map(p => p.name.padEnd(6)).join(' '));
