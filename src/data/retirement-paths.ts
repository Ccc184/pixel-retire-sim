...
const aiSymbiote: RetirementPath = {
  ...
  initialEffect: (state) => {
    ...
    state.lifeLog.push('{startAge}岁这年你试着用AI帮你写代码，它居然跑通了，省了你两个小时。白天上班，晚上琢磨怎么让这东西干活。别人拿它写打油诗的时候，你已经在调提示词跑模型了。这还只是副业，但屏幕上那行跑通的代码让你觉得，有什么东西要变了。');
  },
  ...
  openingMonologues: [
    ...
    { ageRange: [46, 50], texts: [
      'AI不再是新东西了，翻出{startAge}岁第一个prompt，哭笑不得。',
    ...
    { ageRange: [56, 60], texts: [
      ...
      '偶尔想如果{startAge}岁没选这条路会怎样，没答案也不需要了。',
  ...
  sideHustleMonologues: [
    ...
    { ageRange: [36, 45], texts: [
      ...
      '{age}岁才认真做AI副业，晚了{years}年，但你有客户有存款兜底，不怕晚出发。',
    ]},
  ],
...
const chainNative: RetirementPath = {
  ...
  initialEffect: (state) => {
    ...
    state.lifeLog.push('{startAge}岁这年你把第一笔存款的三成换成了加密货币。室友说你疯了，你爸说你被传销骗了。白天照常上班，凌晨三点盯着K线手心出汗。你抄下24个助记词的那晚，把那张纸看了不下二十遍——这串单词是你的身家，丢了就什么都没了。');
  },
  ...
  sideHustleMonologues: [
    ...
    { ageRange: [36, 45], texts: [
      ...
      '{age}岁才辞职All In，不是钱不够，是怕了{years}年终于想明白怕就永远不出发。',
...
const digitalNomad: RetirementPath = {
  ...
  initialEffect: (state) => {
    ...
    // pathFaith 由 selectRetirementPath() 统一设置为40，此处不再覆盖
    state.passiveIncome = 0;
    state.lifeLog.push('{startAge}岁这年你在Upwork上投了六十多份提案，终于拿到第一个远程客户——时薪180块。白天照常上班，晚上用AI工具帮深圳创业公司做网站。你没辞职，但已经在研究清迈的物价和巴厘岛的共享空间了。登机箱放在床边，你在等一个出发的日子。');
  },
  ...
  sideHustleMonologues: [
    ...
    { ageRange: [36, 45], texts: [
      '{age}岁还在上班但远程客户稳定五年了，不是不敢辞是父母需要医保给他们安全感。',
      '{age}岁终于辞职，算过所有数字买好国际医保，登机箱里一台电脑一件冲锋衣。',
...
const superIP: RetirementPath = {
  ...
  initialEffect: (state) => {
    ...
    state.lifeLog.push('{startAge}岁这年你注册了第一个内容账号。白天上班，晚上在出租屋支起补光灯剪视频到凌晨两点。你不知道要发什么，但你知道一件事：如果十年后还需要投简历找工作，你就输了。第一条视频23个赞，18个来自亲戚，但有5个来自陌生人。');
  },
  ...
  openingMonologues: [
    ...
    { ageRange: [26, 30], texts: [
      ...
      '行业活动被认出来，小姑娘说"看着你内容长大的"，你才{age}岁。',
  ...
  sideHustleMonologues: [
    ...
    { ageRange: [36, 45], texts: [
      '{age}岁还在上班但内容矩阵月入是工资五倍，同事以为你是普通中层。',
      '{age}岁终于辞职，不是冲动，是再不下场窗口期就关了。',
      '{age}岁才认真做内容比同行晚不止一轮，但你有真实阅历有人愿意听过来人说话。',
    ]},
  ],
...
const silverEconomy: RetirementPath = {
  ...
  initialEffect: (state) => {
    ...
    state.lifeLog.push('{startAge}岁这年你做了所有人不理解的决定：周末回老家，用AI陪伴机器人帮邻居照顾独居老人。你妈说"供你上大学不是让你回来伺候老人的"。你没反驳。帮张奶奶和国外孙子视频通话那一刻，她攥着你的手不放。她的手很干很薄，像一片秋天的叶子。');
  },
  ...
  sideHustleMonologues: [
    ...
    { ageRange: [36, 45], texts: [
      '{age}岁还在公司但养老服务站已经三家了，同事以为你周末"搞投资"。',
      '父亲走了才辞职，葬礼上想你照顾了那么多别人父母却没多陪他。',
      '{age}岁All In养老比同行晚十几年，但你带来了大公司的管理经验和流程思维。',
    ]},
  ],
...
const bioGambler: RetirementPath = {
  ...
  initialEffect: (state) => {
    ...
    state.stockPct = 0;
    state.bankDepositPct = 100;
    state.lifeLog.push('{startAge}岁这年你开始了"延寿计划"：白天上班，晚上研究抗衰论文、吃NMN、测基因。四成积蓄买了生物科技股。室友说你疯了。书桌上补剂瓶越堆越多，你在每个瓶盖上贴了标签和服用时间。你自己也不确定是不是疯了，但那篇小鼠寿命延长30%的论文你反复读了七遍。');
  },
  ...
  sideHustleMonologues: [
    ...
    { ageRange: [36, 45], texts: [
      '{age}岁还在上班但生物投资是年薪二十倍，同事聊养老金你想退休这个词可能不存在。',
      '{age}岁辞职，心脏检查出异常让你重新审视时间，要下场近距离参与这个行业。',
      '{age}岁才系统投生物科技比{startAge}岁All In晚{years}年，但你有资本有判断力，长坡厚雪不怕晚。',
    ]},
  ],
