    ...
    options: [
      {
        ...
        stateEffect: (s: GameState) => {
          ...
          if (roll < 0.15) {
            ...
            s.happiness = clamp(s.happiness + 25, 0, 100);
            s.pathFaith = clamp(s.pathFaith + 18, 0, 100);
            s.lifeLog.push(`破圈的热度没有两周散去——它持续了整整三个月。你接了四个广告、出了一本畅销书、上了一档爆款综艺。付费课程上线第一天卖了200万。你的被动收入（课程+版权+长尾广告）直接覆盖了两年生活费，IP声誉也涨到了行业顶端。小棠给你发了一条私信，只有四个字："我就知道。"后面跟着一个${s.startAge || 22}岁时你卡壳重说的视频截图——那是她关注你的第一天。你看着后台那个"矩阵总粉丝"突破500万的曲线，第一次觉得"超级IP"这四个字名副其实。你可以退休了，但你不退——因为你正在巅峰。`);
          } else if (roll < 0.5) {
            // 小成功
